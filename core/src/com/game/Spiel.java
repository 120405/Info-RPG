...
public class Spiel extends Game {
    ...
    public void createHealthBars(boolean monster) {
        ...
        if(monster) {
        shapeRenderer.rect(Gdx.graphics.getWidth() - (20 + fight.getMonster().getFullLP()), 20, fight.getMonster().getLP(), 20);
        }
        shapeRenderer.end();
        ...
        if(monster) {
        shapeRenderer.rect(Gdx.graphics.getWidth() - (19 + fight.getMonster().getFullLP()), 19, fight.getMonster().getFullLP() + 1, 21);
        }
        shapeRenderer.end();
        batch2.begin();
        font.draw(batch2, fight.getHero().getName(), (int) (fight.getHero().getFullLP() / 2), 61);
        if(monster) {
        font.draw(batch2, fight.getMonster().getName(), Gdx.graphics.getWidth() - (45 + (int) (fight.getMonster().getFullLP() / 2)), 61);
        }
        batch2.end();
    }
    public void dispose() {
