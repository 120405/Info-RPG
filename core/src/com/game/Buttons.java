	...
	public Buttons(String displayedText, Stage stage, final String action, double x, double y, Color color) {
		...
		button.addListener(new ChangeListener() {
			...
			public void changed(ChangeListener.ChangeEvent event, Actor actor) {
				...
				case "game":
					if(!game.ngame) {
						if(game.lgame) {
							...
							game.getDatabase().save(game.getItems(), game.fight.getHero().getLP(),
									game.fight.getMonster().getLP(), game.getMoney());
						}
						game.getInventoryGUI().getInventory().clear();
						game.getInventoryGUI().getTable().clear();
						if(!game.getDatabase().doesGameExist()) {
							game.getDatabase().setGameID(0);
						} else {
							game.getDatabase().setGameID(2);
						}
				...
				case "quit":
					if (game.saveEnabled) {
						if (!game.getDatabase().doesGameExist(1)) {
							game.getDatabase().setGameID(1);
