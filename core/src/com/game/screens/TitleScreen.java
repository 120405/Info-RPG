...
public class TitleScreen extends ScreenAdapter {
	...
	public void render(float delta) {
		...
		font.draw(batch, name, (int) (Gdx.graphics.getWidth() / 2) - font.getData().padRight * 15,
				(int) (Gdx.graphics.getHeight() / 2) + 250);
		batch.end();
	...
	public void show() {
		// (Spiel.INSTANCE.getDatabase().isSaveFileEnabled() || rausnehmen, weil sonst
		// load game auch wenn saveEnabled = false
		
		if (Spiel.INSTANCE.getDatabase().doesGameExist(1)
				&& (Spiel.INSTANCE.getDatabase().isSaveFileEnabled() || Spiel.INSTANCE.saveEnabled)) {
