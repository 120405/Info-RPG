...
public class GUI {
	...
	public GUI() {
		...
		Window.WindowStyle windowStyle = new Window.WindowStyle(new BitmapFont(), Color.WHITE,
				new SpriteDrawable(new Sprite(img)));
	...
	/*
	 * public void addComponent() { window.add(); for later added components }
	 */
	public void show() {
	...
	public void addInventory(Table table, GUI_Item[][] items) {
		...
		for (int a = 0; a < 4; a++) {
			for (int i = 0; i < 8; i++) {
				...
				// table.add(items[a][i].getItemStack().getChild(0)).size(30, 30); just
				// background
